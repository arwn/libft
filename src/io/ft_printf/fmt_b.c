...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/04/16 10:53:23 by zfaria            #+#    #+#             */
/*   Updated: 2019/04/16 10:57:51 by zfaria           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		*fmt_b(t_fmtarg *arg, va_list varg)
{
	...
	if (ft_isupper(arg->funcc))
		...
		arg->cas = 0;
	if (arg->longflag)
		res->str = g_stypes[LL](arg, varg, 2);
	else if (arg->shortflag == 1)
		res->str = g_stypes[H](arg, varg, 2);
	else if (arg->shortflag == 2)
		res->str = g_stypes[HH](arg, varg, 2);
	else
		res->str = g_stypes[R](arg, varg, 2);
	origzero = 0;
	if (res->str[0] == '0')
		origzero = 1;

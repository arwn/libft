...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/04/16 10:18:51 by zfaria            #+#    #+#             */
/*   Updated: 2019/04/16 10:29:26 by zfaria           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		*fmt_o(t_fmtarg *arg, va_list varg)
{
	...
	if (ft_isupper(arg->funcc))
		...
		arg->cas = 0;
	if (arg->longflag)
		res->str = g_stypes[LL](arg, varg, 8);
	else if (arg->shortflag == 1)
		res->str = g_stypes[H](arg, varg, 8);
	else if (arg->shortflag == 2)
		res->str = g_stypes[HH](arg, varg, 8);
	else
		res->str = g_stypes[R](arg, varg, 8);
	origzero = 0;
	if (res->str[0] == '0')
		origzero = 1;
